...
public interface IncomeRep extends JpaRepository<Income, Integer> {

	@Query(value = "SELECT a.id,a.church_id,a.type_id,b.name typename,a.amount," +
				 "a.issued,a.remark,a.updated " +
				 " FROM income a LEFT OUTER JOIN budget b ON a.type_id=b.id " +
				 " WHERE a.church_id=:churchId AND a.issued BETWEEN :startDt AND :endDt " +
				 " ORDER BY a.issued DESC"
			, nativeQuery = true)
	List<iIncome> searchByChurchIdAndIssuedBetween(@Param("churchId") int churchId,
